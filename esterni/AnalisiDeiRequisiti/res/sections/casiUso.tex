...
\begin{itemize}
	...
	\item\textbf{Estensioni:}
	\begin{itemize}
		\item[$-$] visualizzazione errore: lo smartphone è troppo distante dal tag NFC per effettuare correttamente la scansione (UC19.1).
	\end{itemize}
\end{itemize}
\subsubsection{ UC19.1 - Visualizzazione errore scansione: tag NFC troppo distante}
\begin{itemize}
	\item\textbf{Attori Primari:} dipendente.
	\item\textbf{Descrizione:} l’utente visualizza un messaggio di errore in quanto tenta di effettuare una scansione del tag NFC, ma appoggia lo smartphone non abbastanza vicino al sensore.
	...
	al tag NFC.
	\item\textbf{Precondizione:} l’utente ha tentato di effettuare una scansione del tag NFC.
...
\begin{itemize}
	...
	\end{itemize}
	\item\textbf{Scenario principale:} l’utente appoggia lo smartphone sul tag NFC e visualizza nell'applicazione mobile lo stato della postazione. Nel caso in cui quest'ultima risulta occupata, viene mostrato il nome del dipendente che la occupa. 
...
\begin{itemize}
	...
	\item\textbf{Estensioni:} 
	\begin{itemize}
		\item[$-$] visualizzazione errore: se l'utente sposta lo smartphone dal tag NFC (ad esempio per una telefonata), dopo un timeout di qualche minuto, viene visualizzato un messaggio di errore (UC21.2);
		\item[$-$] visualizzazione errore: se l'utente alza lo smartphone dal tag NFC e abbandona la postazione in modo prematuro rispetto al tempo di fine prenotazione, dopo mezzora viene visualizzato un messaggio di errore e disdetta la prenotazione per il tempo successivo (UC21.4).
	\end{itemize}
\end{itemize}
...
\begin{itemize}
	\item\textbf{Attori Primari:} dipendente.
	\item\textbf{Descrizione:} l’utente visualizza un messaggio di errore in quanto sposta lo smartphone dal tag NFC.
	\item\textbf{Scenario principale:} viene visualizzato un messaggio di errore che consiglia di riposizionare lo smartphone correttamente sul tag NFC.
	\item\textbf{Precondizione:} l’utente sposta il proprio smartphone dal tag NFC.
	\item\textbf{Postcondizione:} viene visualizzato un messaggio di errore specifico.
\end{itemize}
...
\begin{itemize}
	...
	\item\textbf{Scenario principale:} dopo 30 minuti, viene visualizzato un messaggio di errore in cui si notifica
...
\subsection{ UC - Avviso pre-prenotazione }
\begin{itemize}
	\item\textbf{Attori Primari:} dipendente.
	\item\textbf{Descrizione:} 
	\item\textfb{Scenario Principale:} un'ora prima dell'inizio della prossima prenotazione il dipendente riceve un notifica dall'applicazione che gli indica l'imminenza dell'evento.
...
\subsection{ UC - Avviso disdetta automatica prenotazione }
\begin{itemize}
	\item\textbf{Attori Primari:} dipendente.
	\item\textbf{Descrizione:} 
	\item\textfb{Scenario Principale:} 30 minuti dopo l'inizio della prenotazione, se l'utente non ha ancora appoggiato il telefono sul tag NFC:
	\begin{enumrate}

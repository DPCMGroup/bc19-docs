...
\begin{itemize}
	\item \textbf{registerModel(): void}: associa il Model al Presenter; 
	\item \textbf{scanTagNFC(workId:Int): void}: Si occupa di richiedere al modello le informazioni riguardanti la postazione, così poi da ritornarle alla vista; 
	\item \textbf{sanitize(workId:Int): void}: Si occupa di richiedere al modello di igienizzare una postazione, così poi da ritornarlo alla vista. 
\end{itemize}

La classe MainModel offre i seguenti metodi:
\begin{itemize}
	\item \textbf{getStatus(workId:Int): DataWorkstation}: Si occupa di richiedere al service le caratteristiche di una determinata postazione;
	\item \textbf{setSanitize(workId:Int): void}: Si occupa di richiedere al service di igienizzare una determinata postazione.
\end{itemize}
...
Il Model, per igienizzare una postazione, farà prima una chiamata alla classe DataWorkstation con il metodo \textit{getStatus} per ottenere l'id della Workstation e poi alla classe User con il metodo \textit{getId} per ottenere l'id dello user.
